...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE kontak SET id = ?, nama = ?, email = ?, notelp = ?, ProgramStudi = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $notelp, $ProgramStudi, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <input type="text" name="ProgramStudi" value="<?=$contact['ProgramStudi']?>" id="title">
